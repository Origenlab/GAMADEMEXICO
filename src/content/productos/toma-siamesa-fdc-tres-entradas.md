---
title: "Toma Siamesa FDC Tres Entradas 2.5\" NH"
description: "Toma siamesa FDC con tres entradas 2.5\" NH para alto caudal. Capacidad 1,500 GPM total. Válvulas clapper. UL/FM NFPA 14."
categoria: "conexiones-herrajes"
subcategoria: "siamesa"
imagen: "/img/conexiones-herrajes/toma-siamesa-tres-entradas-frontal.avif"
galeria:
  - "/img/conexiones-herrajes/toma-siamesa-tres-entradas-lateral.avif"
  - "/img/conexiones-herrajes/toma-siamesa-tres-entradas-entradas.avif"
  - "/img/conexiones-herrajes/toma-siamesa-tres-entradas-instalacion.avif"
  - "/img/conexiones-herrajes-contra-incendios/toma-siamesa-tres-entradas-frontal.avif"
  - "/img/conexiones-herrajes-contra-incendios/toma-siamesa-fdc-bronce-contra-incendios-bomberos-dos-bocas-01.avif"
certificaciones: ["UL Listed", "FM Approved", "NFPA 14"]
flujo: "1,500 GPM total"
material: "Bronce fundido"
marca: "Elkhart Brass"
modelo: "6-A"
precioReferencia: "$985.00 USD + IVA"
destacado: false
orden: 23
---

Toma siamesa FDC de alto caudal con tres entradas 2.5" NH para sistemas que requieren alimentación masiva. Ideal para edificios grandes y complejos industriales.

## Características principales

- Tres entradas: 2.5" NH hembra swivel
- Capacidad: 500 GPM por entrada (1,500 GPM total)
- Válvulas clapper independientes
- Salida: 5" o 6" bridada
- Mayor capacidad de alimentación

## Materiales

- Cuerpo: bronce fundido de alta calidad
- Clappers: bronce con asiento de goma
- Conexiones: NH estándar

## Aplicaciones

- Centros comerciales grandes
- Complejos industriales
- Almacenes de alta ocupación
- Instalaciones de alto riesgo
