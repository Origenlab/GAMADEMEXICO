---
title: "Boquilla Marine USCG-150 USCG SOLAS MED C95800 60-150 GPM"
description: "Boquilla Elkhart Brass Marine USCG-150 bronce marino C95800. Triple cert USCG SOLAS MED. 60-150 GPM. Buques, plataformas offshore y marina mercante México."
categoria: "boquillas"
subcategoria: "certificada"
imagen: "/img/productos/boquilla-certificada-fm-ul-contra-incendios-bronce-fondo-blanco-01.avif"
galeria:
  - "/img/productos/boquilla-certificada-fm-approved-perfil-lateral-08.avif"
  - "/img/productos/boquilla-certificada-ul-fm-vista-frontal-09.avif"
  - "/img/productos/boquilla-certificada-fm-camion-bomberos-manguera-11.avif"
  - "/img/productos/boquilla-certificada-ul-listed-estacion-bomberos-12.avif"
  - "/img/boquillas-contra-incendios/boquilla-certificada-uscg-solas-monitor-frontal.avif"
certificaciones: ["USCG", "SOLAS", "MED"]
flujo: "60 - 150 GPM"
material: "Bronce marino"
marca: "Elkhart Brass"
modelo: "Marine USCG-150"
precioReferencia: "$595.00 USD + IVA"
destacado: true
indexable: true
orden: 28
---

La **boquilla Marine USCG-150 de Elkhart Brass** es la boquilla de intervención marítima certificada simultáneamente por la **Guardia Costera de los Estados Unidos (USCG)**, la convención **SOLAS (Safety of Life at Sea)** y la **Directiva de Equipos Marinos de la Unión Europea (MED 2014/90/EU)**. Fabricada en aleación de bronce marino C95800 (aluminio-bronce), resistente a la corrosión por agua salada, cloruros y atmósferas marinas, con cuatro posiciones de flujo entre 60 y 150 GPM, es el equipo de protección contra incendios obligatorio para buques de bandera estadounidense y aceptado sin restricción en buques que navegan en aguas internacionales bajo el Convenio SOLAS.

## Especificaciones Técnicas

| Característica | Especificación |
|----------------|----------------|
| Marca | Elkhart Brass |
| Modelo | Marine USCG-150 |
| Rango de flujo | 60 – 95 – 125 – 150 GPM (227 – 360 – 475 – 568 LPM) |
| Presión de trabajo | 100 PSI (6.9 BAR) |
| Presión de prueba hidrostática | 300 PSI (20.7 BAR) |
| Entrada | 1.5" NST hembra con swivel de giro libre |
| Material del cuerpo | Bronce marino aleación C95800 (aluminio-bronce) |
| Selector de flujo | 4 posiciones con indexado positivo |
| Patrones de descarga | Chorro sólido — niebla cónica hasta 90° |
| Peso | 2.4 kg (5.3 lbs) |
| Temperatura de servicio | –18 °C a +65 °C |
| Resistencia a corrosión | Agua salada, cloruros, atmósfera marina |
| Certificaciones | USCG (46 CFR 76.10), SOLAS II-2, MED 2014/90/EU |
| Marcado | Número de aprobación USCG grabado permanentemente |

## Certificaciones y Normas

La certificación **USCG bajo 46 CFR 76.10** es un requisito legal para todo equipo de protección contra incendios instalado en buques de bandera estadounidense; sin esta aprobación, el equipo no puede ser montado en operación legal y el buque puede ser detenido durante una inspección de puerto (PSC). La **SOLAS**, adoptada por la Organización Marítima Internacional (OMI) y en vigor en más de 160 países, establece los estándares mínimos de equipo contra incendios para buques de navegación internacional; la certificación SOLAS II-2 de la Marine USCG-150 garantiza su aceptación sin condicionamientos en cualquier puerto del mundo. La **Directiva MED 2014/90/EU** (marcado de timón) extiende la aceptación a buques que navegan bajo pabellón europeo y puertos de la Unión Europea. El uso de bronce marino C95800 no es una preferencia estética: la aleación aluminio-bronce es una de las pocas que satisface simultáneamente los requisitos de resistencia a la corrosión en agua salada, resistencia mecánica bajo fatiga por vibración del buque y compatibilidad con conexiones NST estándar de bomberos portuarios.

## Características Principales

- **Triple certificación USCG / SOLAS / MED** con números de aprobación grabados permanentemente para verificación inmediata durante inspecciones de capitanía
- **Bronce marino aleación C95800** (aluminio-bronce) — máxima resistencia a corrosión en agua salada, cloruros y atmósferas marinas de largo plazo
- **Cuatro posiciones de flujo indexadas** (60, 95, 125, 150 GPM) para adaptarse a las diferentes demandas de enfriamiento, ataque y dilución a bordo
- **Swivel de giro libre** en 1.5" NST para acoplamiento rápido en condiciones de emergencia, con cubierta mojada o con guantes
- **Patrón de niebla de 90°** para protección del tripulante en espacios confinados de sala de máquinas y bodegas de carga
- **Listada en USCG Equipment List** (cgmix.uscg.mil) con número verificable en línea por inspectores y armadores
- **Documentación marítima completa** — certificado USCG, conformidad SOLAS y declaración MED incluidos con cada unidad
- **Resistencia a vibración continua** del casco del buque bajo diseño y pruebas específicas para aplicación naval

## Aplicaciones

- Buques de carga general, portacontenedores y graneleros de bandera estadounidense sujetos a 46 CFR
- Cruceros, ferries de pasajeros y buques ro-ro que operan rutas internacionales bajo SOLAS
- Buques tanque (petroleros, gaseros, químicos) con requerimientos adicionales de combate de incendios de hidrocarburos
- Plataformas offshore fijas y flotantes en el Golfo de México, el Mar del Norte y el Golfo Pérsico
- Remolcadores y embarcaciones de servicio portuario que requieren certificación USCG para operar en aguas jurisdiccionales de EE. UU.
- Buques de la Marina Mercante Mexicana en tránsito internacional que deben cumplir con SOLAS y son inspeccionados bajo el Memorándum de Tokio y el Memorándum de París (PSC)

## Normas Aplicables

| Norma | Organismo | Alcance |
|-------|-----------|---------|
| **46 CFR 76.10** | USCG | Equipos de incendio aprobados para buques de bandera estadounidense |
| **SOLAS II-2** | OMI | Capítulo de protección contra incendios; requisitos de equipo en buques de navegación internacional |
| **MED 2014/90/EU** | Unión Europea | Directiva de Equipos Marinos; marcado de timón para buques bajo pabellón europeo |
| **NFPA 1405** | NFPA | Guía para cuerpos de bomberos que operan en instalaciones marinas y puertos |
| **NFPA 15** | NFPA | Sistemas de agua-spray; aplicable en plataformas offshore y buques tanque |
| **API 2030** | API | Guía para protección contra incendios en la industria petrolera; aplicable en plataformas |
| **NOM-002-STPS-2010** | STPS México | Aplica en instalaciones portuarias terrestres y talleres navales en México |

## Productos Relacionados

- [Boquilla Certificada PEMEX Series PX-200 NRF-PEMEX](/productos/boquilla-certificada-pemex-petroleo) — boquilla NRF-PEMEX para plataformas offshore en instalaciones con requisito PEMEX
- [Boquilla Industrial ATEX AX-125 Zona 1/2](/productos/boquilla-industrial-antideflagrante-atex) — boquilla antideflagrante para zonas clasificadas en plataformas y buques tanque
- [Manguera Blindex Storz Refinería 2.5"](/productos/manguera-blindex-storz-refineria) — manguera de alta presión para sistemas de brigada marítima e industrial
- [Monitor Cuello Cisne Certificado PEMEX](/productos/monitor-cuello-cisne-certificado-pemex) — monitor fijo para instalaciones offshore y terminales marítimas

## Preguntas Frecuentes

**¿Qué es la aprobación USCG 46 CFR 76.10 y por qué es obligatoria en buques?**
La 46 CFR (Code of Federal Regulations, Título 46) regula la seguridad marítima en EE.UU. La sección 76.10 especifica que todo equipo de protección contra incendios instalado en buques de bandera estadounidense debe tener aprobación previa de la Guardia Costera (USCG). Sin este listado, el equipo no puede instalarse legalmente en operación, y el buque puede ser detenido en cualquier inspección de puerto (Port State Control). El número de aprobación USCG de la Marine USCG-150 es verificable en cgmix.uscg.mil.

**¿La certificación SOLAS es suficiente para operar en puertos mexicanos o requiero certificación adicional?**
SOLAS es el convenio internacional que México ratificó a través de la OMI, y la Secretaría de Marina (SEMAR) la reconoce para buques en navegación internacional. Para buques que operan exclusivamente en aguas nacionales (cabotaje), puede requerirse adicionalmente el cumplimiento de la NOM-SCTSCFI aplicable a embarcaciones nacionales. Consulte con la Capitanía de Puerto correspondiente para confirmar los requisitos específicos de su embarcación.

**¿La aleación C95800 (aluminio-bronce) resiste el agua de mar mejor que el bronce estándar?**
Sí. La aleación C95800 (aluminio-bronce) tiene una resistencia a la corrosión por cloruro y agua de mar significativamente superior al bronce C836 o C932 (latón-estaño) usados en componentes estándar. El aluminio-bronce forma una capa de óxido de aluminio pasivante que resiste la cavitación y la picadura por cloruros, condiciones típicas en sistemas de agua de mar a bordo de buques y en plataformas offshore en el Golfo de México.

**¿La Marine USCG-150 puede usarse también en instalaciones portuarias terrestres?**
Sí. La boquilla puede instalarse en sistemas de brigada de terminales portuarias, astilleros y talleres navales en tierra. En estas instalaciones terrestres en México, también aplica la NOM-002-STPS-2010, que acepta equipos con certificaciones internacionales equivalentes. El bronce marino C95800 es especialmente valorado en entornos costeros donde la corrosión por salinidad ambiental es un factor de deterioro acelerado.

Gama de México es distribuidor autorizado de Elkhart Brass en México y entrega la Marine USCG-150 con certificado de aprobación USCG original, declaración de conformidad SOLAS, declaración MED (cuando aplique), ficha técnica del producto, manual de operación en inglés y español y registro de trazabilidad, brindando además soporte técnico local para equipos de protección contra incendios en el sector marítimo y offshore.
