---
title: "Chiflón Tres Pasos 205-B 1.5\" Elkhart Brass"
description: "Chiflón de tres pasos modelo 205-B de 1.5\" NH. Cerrado, neblina y chorro directo. Flujo 75-125 GPM. Bronce fundido. UL 401."
categoria: "conexiones-herrajes"
subcategoria: "chiflones"
imagen: "/img/conexiones-herrajes/chiflon-205b-1-5-frontal.avif"
galeria:
  - "/img/conexiones-herrajes/chiflon-205b-1-5-posiciones.avif"
  - "/img/conexiones-herrajes/chiflon-205b-1-5-patron.avif"
  - "/img/conexiones-herrajes/chiflon-205b-1-5-operacion.avif"
  - "/img/conexiones-herrajes-contra-incendios/toma-siamesa-tres-entradas-frontal.avif"
  - "/img/conexiones-herrajes-contra-incendios/adaptador-bronce-nh-nst-2-5-frontal.avif"
certificaciones: ["UL 401", "NFPA 1964"]
flujo: "75-125 GPM"
material: "Bronce fundido"
marca: "Elkhart Brass"
modelo: "205-B"
precioReferencia: "$285.00 USD + IVA"
destacado: true
orden: 33
---

Chiflón de tres pasos que permite seleccionar entre cerrado, neblina (fog) y chorro directo (jet). Serie Chief XD con cuerpo forjado para máxima durabilidad.

## Modelo Industrial Estándar (205-B)

- Caudal: 75-125 GPM a 100 PSI
- Presión de operación: 100 PSI
- Disponible en 1.5" NHT y 2.5" NHT

## Serie Chief XD Premium

- Caudal: 160-200 GPM a 50-75 PSI
- Más eficiente a presiones bajas
- Cuerpo forjado (no fundido)
- Mango bale metálico forjado

## Posiciones

- Cerrado (conservación de agua)
- Neblina ancha (wide fog)
- Neblina estrecha (narrow fog)
- Chorro directo (jet stream)

## Materiales

- Bronce fundido libre de mantenimiento
- Aluminio forjado (modelos premium)
- Dientes de acero inoxidable
- Anillos de sello de uretano

## Aplicaciones

- Operaciones de combate de incendios
- Brigadas industriales
- Control de descarga en operaciones ofensivas
- Áreas con solventes químicos (UL 401)
