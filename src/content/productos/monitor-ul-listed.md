---
title: "Monitor UL Listed"
description: "Monitor listado por Underwriters Laboratories con pruebas de seguridad integrales. Aceptado por Protección Civil y aseguradoras en México."
categoria: "monitores"
subcategoria: "ul-listed"
imagen: "/img/monitores-contra-incendios/monitor-ul-listed-contra-incendios-estacion-bomberos-01.avif"
galeria:
  - "/img/monitores-contra-incendios/monitor-ul-listed-fixed-flow-perfil-completo.avif"
  - "/img/monitores-contra-incendios/monitor-ul-listed-fixed-flow-detalle-manometro.avif"
  - "/img/monitores-contra-incendios/monitor-ul-listed-fixed-flow-instalacion-industrial.avif"
certificaciones: ["UL Listed", "NOM-002-STPS-2010"]
flujo: "Según modelo"
material: "Bronce forjado"
marca: "Elkhart Brass"
modelo: "UL-STD-001"
precioReferencia: "Solicitar cotización"
destacado: false
orden: 14
---

La certificación UL Listed es el sello de confianza más reconocido en América del Norte para equipos de seguridad contra incendios. Los monitores con este listado han sido sometidos a pruebas exhaustivas de rendimiento hidráulico, resistencia estructural y seguridad operativa por Underwriters Laboratories.

## Características principales

- Pruebas integrales: rendimiento hidráulico, resistencia a presión, durabilidad, seguridad operativa
- Aceptación nacional por Protección Civil y bomberos
- Compatible con normativa NOM-002-STPS-2010

## Materiales disponibles

- Bronce forjado
- Acero inoxidable
- Aluminio Elk-O-Lite

## Aplicaciones

- Edificios comerciales
- Centros de datos
- Hospitales
- Aeropuertos
- Naves industriales
- Instituciones gubernamentales

## Documentación incluida

- Certificado UL con número de archivo
- Ficha técnica
- Manual de instalación

## Certificaciones

Este monitor cuenta con certificación **UL Listed** y es compatible con la normativa **NOM-002-STPS-2010**, garantizando su aceptación por Protección Civil, bomberos y aseguradoras en México.
