---
title: "Chiflón Tres Pasos 205-B 2.5\" Elkhart Brass"
description: "Chiflón de tres pasos modelo 205-B de 2.5\" NH. Cerrado, neblina y chorro directo. Flujo 150-250 GPM. Bronce fundido. UL 401."
categoria: "conexiones-herrajes"
subcategoria: "chiflones"
imagen: "/img/conexiones-herrajes/chiflon-205b-2-5-frontal.avif"
galeria:
  - "/img/conexiones-herrajes/chiflon-205b-2-5-posiciones.avif"
  - "/img/conexiones-herrajes/chiflon-205b-2-5-patron.avif"
  - "/img/conexiones-herrajes/chiflon-205b-2-5-operacion.avif"
  - "/img/conexiones-herrajes-contra-incendios/chiflon-pistola-elkhart-1-5-frontal.avif"
  - "/img/conexiones-herrajes-contra-incendios/chiflon-turbojet-2-5-frontal.avif"
certificaciones: ["UL 401", "NFPA 1964"]
flujo: "150-250 GPM"
material: "Bronce fundido"
marca: "Elkhart Brass"
modelo: "205-B-25"
precioReferencia: "$385.00 USD + IVA"
destacado: false
orden: 34
---

Chiflón de tres pasos de mayor diámetro para líneas de 2.5" con mayor capacidad de flujo. Ideal para operaciones de ataque exterior y protección de exposiciones.

## Características principales

- Conexión: 2.5" NH
- Tres posiciones: cerrado, neblina, chorro directo
- Flujo: 150-250 GPM a 100 PSI
- Presión de operación: 100 PSI
- Control manual de patrón

## Materiales

- Cuerpo: bronce fundido
- Componentes internos: bronce
- Anillos de sello: uretano

## Posiciones

- Cerrado: conservación de agua
- Neblina ancha: enfriamiento y protección
- Chorro directo: penetración máxima

## Aplicaciones

- Líneas de ataque de 2.5"
- Protección de exposiciones
- Combate exterior
- Brigadas industriales
