---
title: "Boquilla Select-O-Flow DSF-G 2.5\" 125-250 GPM NFPA 1964"
description: "Boquilla Select-O-Flow DSF-G 2.5\" NST bronce satinado XD Elkhart Brass. 3 posiciones 125-175-250 GPM 100 PSI herradura de cierre NFPA 1964."
categoria: "boquillas"
subcategoria: "tipo-pistola"
imagen: "/img/productos/boquilla-tipo-pistola-contra-incendios-ul-listed-32.avif"
galeria:
  - "/img/productos/boquilla-tipo-pistola-contra-incendios-fondo-estudio-33.avif"
  - "/img/productos/boquilla-tipo-pistola-contra-incendios-producto-profesional-34.avif"
  - "/img/productos/boquilla-tipo-pistola-contra-incendios-catalogo-35.avif"
  - "/img/productos/boquilla-tipo-pistola-contra-incendios-vista-derecha-22.avif"
  - "/img/boquillas-contra-incendios/boquilla-pistola-select-o-flow-empunadura.avif"
certificaciones: ["NFPA 1964", "Construcción XD"]
flujo: "125 - 250 GPM"
material: "Bronce satinado"
marca: "Elkhart Brass"
modelo: "DSF-G"
precioReferencia: "$595.00 USD + IVA"
destacado: true
indexable: true
orden: 2
---

La **boquilla Elkhart Brass Select-O-Flow DSF-G de 2½"** es una boquilla de flujo seleccionable de alto caudal fabricada en **bronce satinado** de construcción pesada (*XD — Extra Duty*), certificada bajo **NFPA 1964** y diseñada para líneas de ataque y líneas maestras en operaciones de gran magnitud. Con entrada de **2½" NST** y selector de tres posiciones (125 – 175 – 250 GPM / 473 – 662 – 946 LPM), permite manejar volúmenes de agua casi el doble de los que admite una boquilla estándar de 1½", lo que la convierte en el equipo indicado para *knockdown* rápido en incendios comerciales, industriales y vehiculares pesados. El mecanismo de **herradura de cierre rápido** (*Horseshoe Handle*) y el **swivel permanente de giro libre** completan una boquilla pensada para el operador que trabaja bajo presión extrema.

## Especificaciones Técnicas

| Característica | Especificación |
|----------------|----------------|
| Marca | Elkhart Brass |
| Modelo | DSF-G (*Select-O-Flow*) |
| Construcción | XD (*Extra Duty* — construcción pesada) |
| Tipo de flujo | Seleccionable, 3 posiciones indexadas |
| Posiciones de flujo | 125 – 175 – 250 GPM (473 – 662 – 946 LPM) |
| Presión de trabajo nominal | 100 PSI (7 BAR) |
| Presión máxima de trabajo | 175 PSI (12 BAR) |
| Conexión de entrada | 2½" NST hembra con swivel de giro libre permanente |
| Material del cuerpo | Bronce satinado |
| Mecanismo de cierre | Herradura de cierre rápido (*Horseshoe Handle*) |
| Selector de flujo | 3 posiciones con indexado táctil positivo |
| Dientes de ajuste de patrón | Acero inoxidable, giratorio |
| Patrones de descarga | Chorro sólido directo hasta niebla amplia |
| Empuñadura | *Pistol grip* con antideslizante |
| Longitud total | 305 mm (12") |
| Peso | 2.7 kg (6.0 lbs) |
| Certificaciones | NFPA 1964, Construcción XD |

## Características Principales

- **Entrada 2½" NST** que permite flujos máximos de 250 GPM (946 LPM) imposibles de alcanzar con boquillas de 1½", proporcionando la masa de agua necesaria para controlar rápidamente incendios de gran intensidad calórica
- **Construcción XD (*Extra Duty*)** con mayor espesor de pared que el estándar de la industria, diseñada para resistir el uso intensivo en departamentos de bomberos profesionales y brigadas industriales de alto riesgo
- **Bronce satinado** con resistencia a la corrosión superior al aluminio estándar, apto para ambientes costeros, marinos moderados y zonas con alta concentración de vapores químicos
- **Selector de tres posiciones** (125 / 175 / 250 GPM) con indexado táctil positivo que permite al operador confirmar el caudal seleccionado sin retirar la vista del frente de fuego
- **Herradura de cierre rápido (*Horseshoe Handle*)** que proporciona palanca mecánica óptima para abrir y cerrar la boquilla de forma instantánea bajo la presión de reacción de líneas de 2½"
- **Swivel de giro libre permanente** (*Free Swivel*) en la entrada NST que elimina la torsión de la manguera maestra cuando el operador gira para cubrir distintas exposiciones
- **Dientes giratorios de acero inoxidable** para ajuste suave y preciso del patrón de descarga de chorro sólido a niebla amplia sin necesidad de herramienta
- **Compatible con líneas de ataque de 2½" y con adaptadores** para uso en hidrantes de alto caudal, *wye* de distribución y sistemas de tubería fija en instalaciones industriales

## Aplicaciones

- Líneas de ataque y líneas maestras en incendios estructurales comerciales e industriales de gran superficie
- *Knockdown* inicial rápido en incendios de alta carga de combustible donde las líneas de 1½" resultan insuficientes
- Protección de exposiciones industriales adyacentes durante incendios en plantas de proceso o almacenamiento
- Brigadas de refinería y petroquímica con sistemas hidrantes de alto caudal a 2½"
- Combate de incendios vehiculares pesados (autobuses, vehículos de carga, aeronaves terrestres)
- Operaciones con hidrantes de alto flujo en zonas de alta densidad industrial y comercial
- Sistemas de ataque rápido (*rapid attack*) en unidades automotrices con descarga directa desde el tanque a alta presión

## Normas Aplicables

- **NFPA 1964** — *Standard for Spray Nozzles*: la boquilla Select-O-Flow DSF-G cumple con todos los requisitos de rendimiento hidráulico, construcción, marcado y pruebas de presión para boquillas de aspersión destinadas a operaciones de combate de incendio.
- **NFPA 1901** — *Standard for Automotive Fire Apparatus*: norma de referencia para el equipamiento de unidades automotrices de bomberos; especifica boquillas de alto caudal de 2½" para líneas de ataque montadas en camiones cisterna y plataformas.
- **UL 401** — *Standard for Hose Nozzles for Fire-Fighting Service*: norma UL para la evaluación de resistencia estructural, hermeticidad y rendimiento de boquillas de manguera contra incendio, incluyendo las de entrada de 2½".
- **NOM-002-STPS-2010** — *Condiciones de seguridad contra incendios en los centros de trabajo*: establece los requisitos de equipos manuales de combate de incendio en instalaciones industriales; el uso de boquillas certificadas NFPA 1964 con las capacidades de caudal de la DSF-G es compatible con el cumplimiento de esta norma en ocupaciones de alto riesgo.
- **NFPA 13** — *Standard for the Installation of Sprinkler Systems*: norma de referencia cuando la línea de 2½" se utiliza como complemento de ataque manual en instalaciones con rociadores automáticos en instalaciones industriales de alto riesgo.
- **API RP 2001** — *Fire Protection in Refineries*: práctica recomendada de la industria petrolera que referencia boquillas de alto caudal para sistemas de manguera en refinerías y plantas petroquímicas.

## Productos Relacionados

- [Boquilla Tipo Pistola Phantom XD 15-250 GPM](/productos/boquilla-tipo-pistola) — boquilla de alto rango con selector 4-6 posiciones en aluminio Elk-O-Lite para líneas de 1.5"
- [Boquilla TFT Handline H-VPG-IF Automática 95-300 GPM](/productos/boquilla-pistola-tft-handline-95-300) — boquilla automática de presión constante 100 PSI para suministro variable
- [Boquilla Certificada NFPA 1964 para Bomberos](/productos/boquilla-certificada-nfpa-1964-bomberos) — referencia de certificación NFPA 1964 para especificación en licitaciones municipales
- [Manguera Blindex 2.5" × 30m](/productos/manguera-blindex-2-5-30m) — manguera de suministro 2.5" NH para conectar con la boquilla DSF-G en operaciones de alto caudal

## Preguntas Frecuentes

**¿Por qué especificar una boquilla de entrada 2.5" en lugar de 1.5" para incendios industriales de gran superficie?**
La boquilla DSF-G con entrada de 2.5" permite caudales máximos de 250 GPM que son físicamente imposibles de alcanzar con una boquilla de 1.5", independientemente de la presión del sistema. En incendios con alta carga de combustible —almacenes, plants de proceso, terminales de almacenamiento— la densidad de aplicación de agua en GPM/ft² es el factor determinante para el control del fuego: una línea de 2.5" con el DSF-G puede lograr la tasa de aplicación requerida en un área de ataque hasta el doble que una línea de 1.5", reduciendo el tiempo hasta el control del fuego y el riesgo de involucración de estructuras adyacentes.

**¿Qué ventaja ofrece el selector de flujo indexado del DSF-G frente a una boquilla automática en operaciones de alto caudal?**
En operaciones de alto caudal con líneas de 2.5", el operador necesita certeza absoluta del caudal real que está descargando para coordinar el balance hídrico con el conductor de la bomba. El selector indexado del DSF-G entrega exactamente 125, 175 o 250 GPM a 100 PSI, permitiendo al conductor ajustar la presión de descarga de la bomba con precisión. Una boquilla automática ajusta el orificio de forma continua, lo que dificulta la coordinación cuando el sistema opera con hidrantes de alta variabilidad de presión y el conductor necesita calcular el rendimiento real del circuito hidráulico.

**¿La herradura Horseshoe Handle del DSF-G opera correctamente bajo la fuerza de reacción de una línea de 2.5" a 250 GPM?**
Sí. La herradura del DSF-G fue diseñada específicamente para líneas de 2.5": su geometría proporciona una palanca mecánica que reduce el esfuerzo de cierre bajo la reacción de una línea a 250 GPM y 100 PSI —que puede generar fuerzas de reacción superiores a 50 kg— a un nivel manejable por un solo operador. La construcción XD (Extra Duty) del cuerpo de bronce satinado y el pivote de la herradura están dimensionados para absorber estas fuerzas sin deformación ni fatiga de material en ciclos de uso repetido.

**¿El bronce satinado del DSF-G resiste ambientes químicos industriales sin protección adicional?**
El bronce fundido es inherentemente resistente a la corrosión por humedad, agua salobre y vapores de hidrocarburos, lo que lo hace adecuado para la mayoría de los ambientes industriales sin recubrimiento adicional. Sin embargo, para instalaciones con vapores de ácido sulfhídrico (H₂S), amoníaco (NH₃) en concentraciones altas o ácidos de proceso, se recomienda consultar la tabla de compatibilidad química de Elkhart Brass para el modelo DSF-G, ya que ciertos compuestos pueden atacar los empaques EPDM internos. En entornos con exposición directa y continua a ácidos fuertes, se evalúa la versión con sellos Viton como alternativa.

Gama de México es **distribuidor autorizado** de Elkhart Brass con inventario disponible en bodegas en **CDMX y Querétaro**. Solicite cotización de la boquilla Select-O-Flow DSF-G de 2½" con ficha técnica oficial, certificado de cumplimiento NFPA 1964 y asesoría técnica para proyectos industriales y de bomberos profesionales.
