---
title: "Llave para Hidrante Pentagonal Bronce"
description: "Llave de bronce para operación de hidrantes con vastago pentagonal. Mango largo anti-retroceso. No produce chispa."
categoria: "conexiones-herrajes"
subcategoria: "accesorios"
imagen: "/img/conexiones-herrajes/llave-hidrante-pentagonal-frontal.avif"
galeria:
  - "/img/conexiones-herrajes/llave-hidrante-pentagonal-cabeza.avif"
  - "/img/conexiones-herrajes/llave-hidrante-pentagonal-mango.avif"
  - "/img/conexiones-herrajes/llave-hidrante-pentagonal-uso.avif"
  - "/img/conexiones-herrajes-contra-incendios/adaptador-bronce-nh-nst-2-5-frontal.avif"
  - "/img/conexiones-herrajes-contra-incendios/adaptador-storz-bronce-2-5-frontal.avif"
certificaciones: ["NOM-002-STPS-2010"]
flujo: "N/A"
material: "Bronce fundido"
marca: "Elkhart Brass"
modelo: "HK-5"
precioReferencia: "$145.00 USD + IVA"
destacado: false
orden: 45
---

Llave de bronce para operación de hidrantes con vástago pentagonal. Fabricada en bronce para evitar producción de chispas en ambientes con atmósferas explosivas.

## Características principales

- Cabeza pentagonal para vástagos estándar
- Mango largo para máximo torque
- No produce chispa (bronce antichispa)
- Diseño anti-retroceso
- Resistente a corrosión

## Materiales

- Cuerpo: bronce fundido antichispa
- Mango: bronce sólido
- Acabado: natural resistente a corrosión

## Aplicaciones

- Operación de hidrantes municipales
- Áreas con atmósferas explosivas
- Refinerías y petroquímicas
- Estaciones de gas y combustible
