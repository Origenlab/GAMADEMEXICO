---
title: "Adaptador Aluminio Elk-O-Lite NH 4\" Storz"
description: "Adaptador de aluminio Elk-O-Lite para conversión NH a Storz 4\". 60% más ligero que bronce. Presión 250 PSI. UL Listed."
categoria: "conexiones-herrajes"
subcategoria: "adaptadores"
imagen: "/img/conexiones-herrajes/adaptador-aluminio-elk-o-lite-4-frontal.avif"
galeria:
  - "/img/conexiones-herrajes/adaptador-aluminio-elk-o-lite-4-lateral.avif"
  - "/img/conexiones-herrajes/adaptador-aluminio-elk-o-lite-4-storz.avif"
  - "/img/conexiones-herrajes/adaptador-aluminio-elk-o-lite-4-peso.avif"
  - "/img/conexiones-herrajes-contra-incendios/adaptador-bronce-nh-nst-2-5-frontal.avif"
  - "/img/conexiones-herrajes-contra-incendios/adaptador-storz-bronce-2-5-frontal.avif"
certificaciones: ["UL Listed", "FM Approved"]
flujo: "250 PSI trabajo"
material: "Aluminio Elk-O-Lite forjado"
marca: "Elkhart Brass"
modelo: "D-327-L"
precioReferencia: "$295.00 USD + IVA"
destacado: false
orden: 6
---

Adaptador de aluminio Elk-O-Lite forjado que ofrece la misma resistencia que el bronce con 60% menos peso. Ideal para operaciones donde el peso del equipo es factor crítico.

## Características principales

- Conversión NH hembra a Storz macho 4"
- 60% más ligero que equivalente en bronce
- Presión de trabajo: 250 PSI
- Resistencia igual al bronce fundido
- Acabado anodizado duro

## Materiales

- Cuerpo: aluminio Elk-O-Lite forjado (no fundido)
- Acabado: anodizado duro negro
- Empaque: EPDM de alta resistencia

## Especificaciones Técnicas

| Característica | Especificación |
|----------------|----------------|
| Marca | Elkhart Brass |
| Modelo | D-327-L |
| Material | Aluminio Elk-O-Lite forjado |
| Presión de trabajo | 250 PSI |
| Presión de prueba | 750 PSI |
| Conexión(es) | NH hembra 4" a Storz macho 4" (cuarto de vuelta) |
| Certificaciones | UL Listed, FM Approved |

## Normas Aplicables

- **NFPA 1963** — Roscas para mangueras contra incendios (NH/NST)
- **NFPA 14** — Sistemas de tuberías verticales y mangueras
- **UL Listed** — UL 405: Conexiones para mangueras contra incendios
- **NOM-002-STPS-2010** — Condiciones de seguridad contra incendios

## Aplicaciones

- Operaciones de bomberos
- Líneas de suministro de gran diámetro
- Situaciones donde el peso es crítico
- Brigadas industriales
