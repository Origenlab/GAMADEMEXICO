---
title: "Chiflón Industrial 1\" para Gabinete"
description: "Chiflón de tres pasos de 1\" NH para gabinetes contra incendios. Flujo 30-60 GPM. Bronce fundido. Uso en espacios confinados."
categoria: "conexiones-herrajes"
subcategoria: "chiflones"
imagen: "/img/conexiones-herrajes/chiflon-industrial-1-frontal.avif"
galeria:
  - "/img/conexiones-herrajes/chiflon-industrial-1-gabinete.avif"
  - "/img/conexiones-herrajes/chiflon-industrial-1-patron.avif"
  - "/img/conexiones-herrajes/chiflon-industrial-1-operacion.avif"
  - "/img/conexiones-herrajes-contra-incendios/chiflon-pistola-elkhart-1-5-frontal.avif"
  - "/img/conexiones-herrajes-contra-incendios/chiflon-turbojet-2-5-frontal.avif"
certificaciones: ["UL 401", "NOM-002-STPS-2010"]
flujo: "30-60 GPM"
material: "Bronce fundido"
marca: "Elkhart Brass"
modelo: "205-B-10"
precioReferencia: "$185.00 USD + IVA"
destacado: false
orden: 37
---

Chiflón compacto de 1" para uso en gabinetes contra incendios y líneas de manguera de menor diámetro. Ideal para espacios confinados y primera respuesta.

## Características principales

- Conexión: 1" NH
- Tamaño compacto para gabinetes
- Flujo: 30-60 GPM a 100 PSI
- Tres posiciones estándar
- Fácil operación con una mano

## Materiales

- Cuerpo: bronce fundido
- Componentes internos: bronce
- Anillos: goma resistente

## Posiciones

- Cerrado
- Neblina para protección personal
- Chorro directo para penetración

## Aplicaciones

- Gabinetes contra incendios
- Líneas de 1" para primera respuesta
- Espacios confinados
- Edificios comerciales
