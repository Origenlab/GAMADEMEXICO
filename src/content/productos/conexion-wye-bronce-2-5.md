---
title: "Conexión Y (Wye) Bronce 2.5\" con Válvulas"
description: "Conexión Y de bronce fundido con entrada 2.5\" NH y dos salidas 2.5\" NH. Válvulas de bola de bronce. Presión 200 PSI."
categoria: "conexiones-herrajes"
subcategoria: "wye"
imagen: "/img/conexiones-herrajes/conexion-wye-bronce-2-5-frontal.avif"
galeria:
  - "/img/conexiones-herrajes/conexion-wye-bronce-2-5-lateral.avif"
  - "/img/conexiones-herrajes/conexion-wye-bronce-2-5-valvulas.avif"
  - "/img/conexiones-herrajes/conexion-wye-bronce-2-5-instalacion.avif"
  - "/img/conexiones-herrajes-contra-incendios/conexion-wye-2-5-bronce-frontal.avif"
  - "/img/conexiones-herrajes-contra-incendios/wye-gated-elkhart-01.avif"
certificaciones: ["UL Listed", "FM Approved"]
flujo: "200 PSI trabajo"
material: "Bronce fundido"
marca: "Elkhart Brass"
modelo: "B-97-B"
precioReferencia: "$1,450.00 USD + IVA"
destacado: false
orden: 12
---

Conexión Y tradicional de bronce fundido para aplicaciones donde se requiere máxima durabilidad y resistencia a ambientes corrosivos. Construcción robusta para servicio pesado.

## Características principales

- Entrada: 2.5" NH hembra swivel
- Salidas: (2) 2.5" NH macho con válvulas
- Presión de trabajo: 200 PSI
- Construcción 100% bronce fundido
- Mayor resistencia a corrosión marina

## Materiales

- Cuerpo: bronce fundido de alta calidad
- Válvulas: bola de bronce macizo
- Manijas: bronce con traba positiva

## Aplicaciones

- Ambientes marinos y costeros
- Instalaciones permanentes
- Plataformas offshore
- Terminales marítimas
